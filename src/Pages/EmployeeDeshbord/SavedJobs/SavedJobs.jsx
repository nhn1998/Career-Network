...
const SavedJobs = () => {
    ...
    const { data: jobdata = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5000/employedashboard/savedjobs/${user?.user?.email}`);
    ...
    return (
        <div>
            <div class="flex items-center py-2 overflow-x-auto whitespace-nowrap">
                ...
                <p class="text-sky-700 dark:text-gray-200 ">Saved Jobs</p>
            </div>
            <p className='text-4xl mt-8'>Saved Jobs</p>
            ...
            <section class="container px-4 mx-auto">
                <div class="flex items-center justify-end pr-2 font-bold">
                    ...
                    <span class="">:{jobdata.length}</span>
                </div>
                            ...
                            <div class="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                                      {jobdata?.map(job=>  <tr key={job._id}>
                                            ...
                                            <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                                <Link to={`/jobdetails/${job.jobId}`} className='btn btn-outline btn-primary btn-xs'>View</Link>
                                            </td>
                                            ...
                                            <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                <div class="flex items-center gap-x-6">
                                                    ...
                                                    <div className='tooltip tooltip-right tooltip-error' data-tip="delete">
                                                        <button onClick={()=>handleUnSaveJob(job.applicant_email,job.jobId)} class="text-red-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-red-300 hover:text-red-600 hover ">
